  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900 flex flex-col items-center">
      ...
      <nav className="w-full bg-white shadow-md p-4 fixed top-0 z-50 flex justify-between items-center px-6 md:px-12">
        ...
        <Button variant="outline" className="text-gray-700 hover:text-green-500 hover:bg-white  border-green-500" asChild>
      ...
      <section className="w-full text-center pt-24 pb-16 px-6 md:px-12 flex flex-col items-center">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1 }} className="mt-12">
          <Image src="/images/food.avif" alt="Delicious food" width={800} height={400} className="rounded-lg shadow-lg" />
        </motion.div>
      </section>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full max-w-6xl px-6 md:px-12 mt-8 mb-16">
        {[
          { title: "Fresh & Authentic", desc: "Enjoy homemade meals crafted with passion.", img: "/images/fresh-food.jpg" },
          { title: "Support Local Chefs", desc: "Empower home chefs by ordering directly from them.", img: "/images/support-chefs.jpg" },
          { title: "Easy & Secure Ordering", desc: "Seamless ordering experience with secure payments.", img: "/images/easy-order.jpg" }
        ].map((feature, index) => (
          <motion.div
            key={index}
